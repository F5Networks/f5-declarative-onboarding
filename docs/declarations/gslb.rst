...
For more details on the properties and BIG-IP DO usage, see |gslbglobal| and |gslbgen| in the Schema Reference.  
...
For details on the available properties and BIG-IP DO usage, see |gslbdata| in the Schema Reference.  
...
BIG-IP DO 1.19 added support for Prober pools and GSLB virtual servers to GSLB Servers. For information on Prober pools, see :ref:`prober`. |br| For information on GSLB virtual servers, see the |gslbvipdoc| chapter of the documentation. For BIG-IP DO options and usage, see |gslbvip|.

For details on the available properties and BIG-IP DO usage, see |gslbserver| in the Schema Reference.  

.. IMPORTANT:: Because this example was updated to include a Prober pool and GSLB virtual servers, if you attempt to use the following declaration on a previous version of BIG-IP DO, it will fail.  If you are using a version prior to 1.19, you can remove the Prober pool and virtual server lines (in yellow). 
...
For details on the available properties and BIG-IP DO usage, see |gslbmon| in the Schema Reference.  
...
This example includes each of the available GSLB monitors which you can use as part of a larger BIG-IP Declarative Onboarding declaration.  In this declaration, the monitors are all used in the GSLB Server (see :ref:`the GSLB Server example<gslbserver>`).
...
For details on the available properties and BIG-IP DO usage, see |gslbpp| in the Schema Reference.  
