...
function updateProperty(property, itemToUpdate, propertyName, declarationItem, idsToDelete, configItem) {
    if (property.newId) {
        let value = doUtil.getDeepValue(itemToUpdate, property.newId);
        ...
        if (typeof value !== 'undefined' && value !== null) {
            ...
            // updated
            if (property.newId !== 'name' && idsToDelete.indexOf(property.newId) === -1) {
                idsToDelete.push(property.newId);
            }
        }

        // newId translation that is in a schemaMerge
        if (configItem.schemaMerge
            ...
            const dottedNewId = [configItem.schemaMerge.path.join('.'), property.newId].join('.');
            value = doUtil.getDeepValue(itemToUpdate, dottedNewId);
            if (typeof value !== 'undefined' && value !== null) {
            ...
            if (trans.newId) {
                if (Array.isArray(itemToUpdate[property.id])) {
                    itemToUpdate[property.id].forEach((subObject) => {
                        const value = subObject[trans.newId];
                        if (typeof value !== 'undefined' && value !== null) {
                            subObject[trans.id] = value;
                            delete subObject[trans.newId];
                        }
                    });
                } else {
                    let dottedId = [property.id, trans.id].join('.');
                    let dottedNewId = [property.id, trans.newId].join('.');
                    ...
                    if (property.upLevel) {
                        ...
                        dottedNewId = dottedNewId.split('.').slice(property.upLevel).join('.');
                    }
                    const value = doUtil.getDeepValue(itemToUpdate, dottedNewId);
                    if (typeof value !== 'undefined' && value !== null) {
                        doUtil.setDeepValue(itemToUpdate, dottedId, value);
                        doUtil.deleteKey(itemToUpdate, dottedNewId);
                    }
